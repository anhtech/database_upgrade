...
class PosOrder(models.Model):
    ...
    def _prepare_invoice(self):
        ...
        # Added
        res.update(
            {'discount_type': self.discount_type and self.discount_type or False})
        # end
        return res
        ...
        for order in self.filtered(lambda o: not o.account_move or order.state == 'paid'):
            ...
            def insert_data(data_type, values):
                ...
                if data_type == 'product':
                    key = ('product', values['partner_id'], (values['product_id'], tuple(
                        values['tax_ids'][0][2]), values['name']), values['analytic_account_id'], values['debit'] > 0)
                elif data_type == 'tax':
                    key = ('tax', values['partner_id'],
                           values['tax_line_id'], values['debit'] > 0)
                elif data_type == 'counter_part':
                    key = (
                        'counter_part', values['partner_id'], values['account_id'], values['debit'] > 0)
                else:
                    return
        ...
        for order in orders:
            ...
            for line in order.lines:
                key = (line.product_id, line.price_unit,
                       line.discount, line.order_id.discount_type)
                ...
                if line.tax_ids_after_fiscal_position:
                    ...
                    if line.order_id.discount_type == 'per':
                        ...
                        line_taxes = line.tax_ids_after_fiscal_position.compute_all(
                            line.price_unit - (line.discount or 0.0), currency, line.qty, product=line.product_id, partner=line.order_id.partner_id or False)
