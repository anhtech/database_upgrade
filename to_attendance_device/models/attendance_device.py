...
def isUserChanged(user_id,device_user):
    if user_id.name!=device_user.name:
        print("user_id.name: "+str(user_id.name))
        print("device_user.name: "+str(device_user.name))
    if user_id.privilege!=device_user.privilege:
        print("user_id.privilege: "+str(user_id.privilege))
        print("device_user.privilege: "+str(device_user.privilege))
    if user_id.password!=device_user.password:
        print("user_id.password: "+str(user_id.password))
        print("device_user.password: "+str(device_user.password))
    if user_id.card!=str(device_user.card):
        print("user_id.card: "+str(user_id.card))
        print("device_user.card: "+str(device_user.card))
    if user_id.uid!=device_user.uid or user_id.name!=device_user.name or user_id.privilege!=device_user.privilege or user_id.password!=device_user.password or user_id.card!=str(device_user.card):
        return True
    return False

def isThisUserNew(user_id, device_users):
    for device_user in device_users:
        if user_id.user_id==device_user.user_id:
            return False
    return True
...
class AttendanceDevice(models.Model):
    ...
    deviceID = fields.Char(string="Device ID", required=True, help='The device ID of the attendance device', copy=True)
    ...
    def setUser(self, uid, name, privilege, password, group_id, user_id, card):
        try:
            print("Set USER Request "+str(uid)+", "+str(name))
#             uid = uid + uid_increment
            ...
            raise ValidationError(_('Could not set user into the device %s. Here are the user information:\n'
                                    ...
                                    'user_id: %s\n'
                                    'card: %s\n')
                                  % (self.display_name, uid, name, privilege, password, group_id, user_id, card))
        finally:
            self.enableDevice()
            self.disconnect()
...
#             uid = uid + uid_increment
            ...
            for device_user in device_users:
                ...
                if uid not in existing_user_ids:
                    ...
                    if existing:
                        ...
                        if existing.card != vals['card']:
                            update_data['card'] = vals['card']
    ...
    def action_user_upload(self):
        ...
        for r in self:
            # This changes is to update the users on device before re-download them again (before storing them) -_-
            # Make it real upload -_-
            print("Here 1")
            device_users = r.getUser()
            for device_user in device_users:
                user_id = DeviceUser.search([('device_id', '=', r.id), ('user_id', '=', device_user.user_id), '|', ('active', '=', True), ('active', '=', False)])
                if isUserChanged(user_id,device_user):
                    print("Update !")
                    r.setUser(user_id.uid,user_id.name,user_id.privilege,user_id.password,user_id.group_id,user_id.user_id,user_id.card)
            print("Here 2")
            # Test if there are new users has been created
            # Create the new users if exist
            users_ids = DeviceUser.search([('device_id', '=', r.id), '|', ('active', '=', True), ('active', '=', False)])
            for user_id in users_ids:
                if isThisUserNew(user_id, device_users):
                    print("There is a new user has been created: "+str(user_id.name))
                    r.setUser(user_id.uid, user_id.name, user_id.privilege, user_id.password, user_id.group_id, user_id.user_id, user_id.card)
            print("Here 3")
    ...
    def action_attendance_download(self):
        ...
        for r in self:
            ...
            # try:
            #     sock.connect(("192.168.100.52", 24999))
            #     msg = {}
            #     msg["task"] = "GetDeviceIP"
            #     msg["DeviceID"] = r.deviceID
            #     msg["KEY"] = "BRC"
            #     b = bytearray()
            #     b.extend(map(ord, json.dumps(msg)))
            #     sock.send(b)
            #     while True:
            #         # Receiving from client
            #         responseBytes = sock.recv(999999)
            #         if responseBytes:
            #             break
            #     responseStr = responseBytes.decode("utf-8")
            #     response = json.loads(str(responseStr))
            #     print(str(response))  # TODO remove it later
            #     if response["IP"] and response["PORT"] and response["PASSWORD"] and response["is_ip_updated"]:
            #         r.password = response["PASSWORD"]
            #         r.ip = response["IP"]
            #         r.port = int(response["PORT"])
            #         print("Updated")
            #     sock.close()
            ...
            for attendance in attendance_data:
                if attendance.punch not in attendance_states.keys():
                    if not r.ignore_unknown_code:
                        ...
                        #              ' go to the device setting in Odoo and check "Ignore Unknown Code". (Restarting the device is now in progress ...)')
                        #            % (attendance.punch, r.display_name, r.display_name))
                        raise UserError(_('Error in attendance status code - Restarting the attendance device ...'))
                        #restart the device now or ignore error !!!!!
                ...
                if user_id:
                    ...
                    if not duplicate_attend:
                        temp_timestamp = attendance.timestamp
                        temp_timestamp = temp_timestamp.replace(hour=16, minute=30, second=0)
                        if attendance.timestamp < temp_timestamp:
                            attendance_states_selected = 0
                        else:
                            attendance_states_selected = 1
                        try:
                            vals = {
                                'device_id': r.id,
                                'user_id': user_id.id,
                                'timestamp': utc_timestamp,
                                'status': attendance_states_selected,
                                'attendance_state_id': attendance_states[attendance_states_selected]
                                }
