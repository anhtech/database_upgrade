...
class ResPartner(models.Model):
    ...
    @api.one
    def prepareCustomerPhoneNumber(self):
        phone = self.phone
        if not phone:
            phone = self.mobile
        if not phone:
    ...
    def sendWhatsappMessage(self):
        for record in self:
            phone = record.prepareCustomerPhoneNumber()
            if not phone:
                continue
            ...
            if not is_success:
                ...
                if not new_is_success:
                    raise UserError(_("Error while sending a whatsapp message for the customer: %s" % (str(record.name),)))
            # In case the whatsapp is already opened on another device.
            record.takeOverRequest(chat_api_url, chat_api_token)
            image = open('/opt/odoo12/odoo/ywebh/pos_whatsapp_chatapi/static/src/img/offer.jpeg', 'rb')
            b = bytearray(image.read())
            base64_string = base64.urlsafe_b64encode(b).decode('ascii')
            encoded_body = json.dumps({'phone': phone, 'body': "data:image/jpeg;base64," + base64_string, 'filename': "offer.jpeg"})
            ...
            template = "خصم جنوني مرة بالسنة !!!!!!!!\n\n ولكثرة طلباتكم فتحنا لكم العرض\nمن خلال زيارتكم المحل\nمن خلال الموقع الإلكتروني\nمن خلال حساب الإنستقرام\nمن خلال الواتساب\n\nفرحو أحبائكم وأصدقائكم بهدية راقية من عشق .. ♥️♥️"
            requests.post(str(chat_api_url) + "" + SEND_MESSAGE + "?token=" + str(chat_api_token), data={"phone": phone, "body": template})
            self.env.user.notify_success(message='The whatsapp message has been sent successfully to %s' % (str(record.name),))
